...
# We load the xml file
    def detectFace(self, image,size):
        ...
        faces = self.CLASSIFIER.detectMultiScale(image) * size
        result  = np.array([])
        for face in faces:
            x1,y1,x2,y2 = face[0],face[1], face[0] + face[2],face[3] + face[1]
            result = np.append(result,[x1,y1,x2,y2])
        return np.reshape(result,(-1,4)).astype(int)
